...
6. **管审批** — 三类审批并存:(a) 风险分超阈值时暂停,路由到人工或专业复核;(b) 质量评分不合格时走返工+反思(错误反思式),最多 3 轮;(c) 命中 HITL 灰色地带条件(H1–H6)时挂起当前角色,生成决策包请人裁决,不消耗返工次数。记录所有审批与裁决结果
...
3. **项目结束时**(经验学习式) — 从本次项目轨迹抽取 insights("遇到 X 情况,应该/不应该 Y"),存入 skills/experience/office-research-secretary/ 目录,供下一个项目复用。
