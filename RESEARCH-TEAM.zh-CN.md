...
3. **给团队加了评分和自我进化能力**,做得不好能返工,返工能反思,跑完能沉淀经验,下次不犯同样的错
...
| 角色 | 干什么 | 配了什么工具 |
|---|---|---|
| **科研秘书** | 接活、分活、盯进度、评分验收 | 自动分派、硬管控审批门、六维评分卡 |
| **课题规划师** | 定研究方向、找创新点 | 文献计量分析、STORM 多视角探索 |
| **文献研究员** | 查论文、写综述、找空白 | arXiv 检索、Scholar 检索、PDF 全文解析(GROBID) |
| **学术写作员** | 写论文、改稿 | LaTeX 工具链、引用自动生成、BibTeX 管理、语法检查 |
...
| **实现工程师** | 写代码、跑实验 | 实验跟踪(MLflow)、超参调优(Optuna)、RL 实验库、可复现包导出 |
...
这是这套系统最核心的设计之一。AI 自评会高估自己(论文已证实),所以我们不靠 AI 自己说"我做完了",而是有一套六维评分卡。
...
### 评分依据
- NeurIPS/ICLR/ACL 顶会审稿标准
- 斯坦福 100+ NLP 研究者的 idea 评估方法(Si et al. 2024)
- Sakana AI 的 AI-Scientist 自动评审
...
skills/                          46 个技能
  ...
  _imported/                     从 Anthropic 官方导入(pdf/docx/xlsx/skill-creator)
