...
这份文档记录了为每个角色补充能力的设计:采用哪些方法论、推荐装哪些开源工具、哪些能直接用、哪些只能参考。

## 一、方法论设计

以下方法论已经融入对应角色的"灵魂文档"(SOUL.md)的"思维方式"部分。

### 跨角色的协作设计

- 端到端科研流水线(idea→实验→写作→审稿)的参考实现
- 学术图扩展找创新点 + 评审嵌入 idea 迭代环
- 多视角 persona 提问发现研究角度
- 综述自动生成四段式(检索→大纲→分段起草→评估重写)
- 每条 claim 必须追溯到原文段落 + 页码
- LLM 生成 idea 新颖但同质化严重,必须强制多样性
- 案例库复用(CBR)做自动 ML 实验
- 多阶段结构化评审 + 14B 开源评审模型
- 多角色模拟审稿,复现权威偏见和群体思维
- 科学 claim 验证基准
...
**课题规划师**:学术图扩展(从核心论文出发拉引用网络找创新点);多样性约束(每次生成≥5个 idea 分属不同子领域);多视角圆桌(理论派/工程派/应用派/批评派相互提问)。

**文献研究员**:综述自动生成四段式(严禁一次性整篇生成);claim 溯源(无溯源的断言直接拒绝);多 persona 提问大纲。

**质检员**:多阶段结构化评审(结构化分析→逐 claim 检索证据→论证批评→给建议);多 persona 评审(严格派/宽松派/专家派/批评派各出评审再讨论);claim 级验证(SUPPORT/REFUTE/证据不足);嵌入式评审(不事后审,而是嵌进 idea 迭代环)。

**方法学专家**:CBR 案例库(新实验先检索相似案例);实验闭环(写代码→跑→看结果→调→再跑);因果推断护栏(区分相关性 claim 和因果性 claim)。

**资料员**:三源采集(OpenAlex 元数据 + Semantic Scholar 影响力 + arXiv 最新预印本);evidence pack(存段落级证据包不是论文清单);自动引文网络(NetworkX 存本地引文图);实体库(抽取方法名/数据集名/指标名形成知识图谱)。
...
本配置包所有内容为原创编写,未复制任何 CC BY-NC/GPL 项目的代码与文案。推荐安装的工具均为 MIT/Apache/BSD 协议,可商业使用。GPL 工具仅建议本地使用,不打包进可分发的 skills 目录。
