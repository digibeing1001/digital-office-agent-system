...
> 设计原则：Role/Task 标准化模板 + sequential process 串行编排。SOP 显式化 + 结构化交接产物 + 下游只消费上游结构化产物。
...
2. **用什么标准干？** —— 准入标准（前置条件）+ 准出标准（产物 + 通过条件）必须显式化，下游只消费上游结构化产物，不允许凭空臆造。
...
v2.2 prompt 层是 best-effort。LLM 可以在 self_check 里撒谎（填 `am_i_writing_content: false` 但实际代写了）。唯一可靠的防代写是机械强制，配置详见 `agent-system/docs/cross-platform-deployment-guide.md`（main 分支）：

- **WorkBuddy**：秘书是单 Agent；把 7 个数字员工拆成 7 个独立 WorkBuddy Skill，每个 Skill 独立工具白名单；秘书 Skill 的 `forbidden_tools` 含 `write_doc`/`write_code`；Gate 等待用 automation SQLite 任务队列。
- **Coze**：秘书 + 7 worker 建成 workflow，条件节点解析 dispatch JSON 路由，每节点独立工具白名单，Gate 节点为 HITL 人工审批节点。
- **LangGraph**：`interrupt_before` 配置在所有 Gate 节点 + `Checkpointer` 持久化恢复；secretary Agent 的 `tools=[dispatch_tool]` 无产出工具。
- **Claude Code / Trae**：秘书是 Subagent dispatcher，`tools=[Task, Read]` 无 Write/Edit；Gate 等待用 permission 系统。
- **Hermes / OpenClaw**：bindings 层 `deny: [write_doc, write_code]`；sandbox scope 隔离；state_guard 拦截非法状态跳转。

机械层未配置时，秘书必须在首轮 `execution_plan` 中声明 `"mechanical_enforcement_status": "not_configured"`，让用户知晓当前仅 prompt 层保护。

### 能力从哪来

采用「三层员工模型」：秘书是控制面（Control Plane），数字员工是执行面，技能是能力通道（Skill Lane）——秘书不下场干活，只调度。采用「思维方式 · 边界 · 操作循环」结构化范式，把调度官的思考路径显式化。采用 Role/Task 标准化模板 + SOP 编排清单，让交接产物结构化、可追溯。
...
6. SOP 编排：按写作 SOP 串行编排，每步有准入/准出标准
7. 任务卡管理：每个任务强制 `description + expected_output + agent + output_file`
...
- 下游只能消费上游的结构化产物（Scratchpad 卡片 / 引用源台账 / 研究简报 schema），**不允许凭空臆造**
...
- v2.0（深化 Role/Task 标准化模板 + SOP 编排清单 + 准入准出标准 + 下游只消费结构化产物约束）
