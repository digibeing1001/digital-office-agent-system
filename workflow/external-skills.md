...
> 第三方/外部技能清单（v2.5 版本，含下载状态标注 + 整合回 7 角色架构后的技能索引）。
...
### 多 Agent 编排范式
- **设计原则**：Role/Task/Crew 抽象可直接迁移到 TEAM.md 的角色卡设计，Research Analyst→Content Writer→Editor 串行交接
- **承载角色**：秘书

### LangGraph
- **设计原则**：StateGraph 状态控制，支持审校→打回→重写回路（图结构状态机）
- **承载角色**：秘书

### SOP 驱动结构化交接
- **设计原则**：SOP 驱动 + 结构化交接产物（PRD→设计文档），中文社区维护活跃
- **承载角色**：秘书/大纲师

### 多角度提问检索写作
- **设计原则**：多角度提问驱动检索→大纲→分节撰写，研究-写作分离，专攻长文
- **承载角色**：研究员/大纲师

### AutoGen
- **设计原则**：Agent 间对话式协作（一个 Agent 调用另一个 Agent）
- **承载角色**：秘书

### Planning-Writing 双层分离
- **设计原则**：Planning Agents（只写 Scratchpad）+ Writing Agents（落正文）双层分离
- **承载角色**：秘书
...
- **对应吸收**：吸收 1 AI 自评偏见消除 / 吸收 2 原子化事实核查 / 吸收 4 Evals 可演进
- **集成方式**：方法论参考——审查员四层金字塔审查可参考其指标设计思想。如团队后续引入代码工程化，可下载集成做自动化评估
...
- **对应吸收**：吸收 4 Evals 可演进
- **集成方式**：方法论参考——审查员改 prompt 时可参考其评估矩阵思想，防止"修了 A 味、坏了事实性"。如团队后续引入 CI/CD，可下载集成
- **边界**：Node.js CLI 工具；当前团队无 CI/CD 流水线，登记为参考

---

## 十一、撰稿人承载（标题生成，整合自 08-title-engineer）

> 本节为 v2.5 更新，标题工程能力从独立的 08-title-engineer 整合回 04-writer，作为写手的内部能力。

### 写手标题技能（04-writer）

| 技能/工具 | 用途 | 优先级 | 状态 |
|---|---|---|---|
| 爆款标题公式库 | 8 大标题公式（悬念/数字/对比/痛点/反常识/权威/故事/清单） | P0 | 内置 |
| 标题 A/B 测试 | 推荐 2-3 个最佳候选 + 测试策略 | P1 | 内置 |
| 公众号摘要生成 | 15-30 字 secondary title | P0 | 内置 |
| 封面图建议 | 风格+配色+文字位置 | P2 | 内置 |

---

## 十二、秘书数据分析技能（整合自 09-data-analyst）

> 本节为 v2.5 更新，数据分析能力从独立的 09-data-analyst 整合回 00-secretary，作为秘书的扩展职责。

### 秘书数据复盘技能（00-secretary）

| 技能/工具 | 用途 | 优先级 | 状态 |
|---|---|---|---|
| 四维归因分析 | 选题/标题/内容/时间贡献归因 | P0 | 内置 |
| 竞品对标分析 | 同领域 Top 10 账号爆款分析 | P1 | 内置 |
| 策略迭代引擎 | 选题/标题/内容/时间四方向优化建议 | P0 | 内置 |
| 公众号后台 API | 自动采集阅读/互动/涨粉数据 | P1 | 待集成 |
| 知乎创作者数据 | 自动采集浏览/互动/关注数据 | P1 | 待集成 |

---

## 十三、运营策略技能（整合自 10-ops-strategist）

> 本节为 v2.5 更新，运营策略能力从独立的 10-ops-strategist 拆分整合回 01-topic-scout（系列规划）和 07-formatter（CTA/涨粉）。

### 选题官运营技能（01-topic-scout）

| 技能/工具 | 用途 | 优先级 | 状态 |
|---|---|---|---|
| 系列化内容规划 | 系列主题设计+节奏控制+前后衔接 | P0 | 内置 |
| 发布日历管理 | 周/月发布节奏+热点预留+节日营销 | P1 | 内置 |

### 排版师运营技能（07-formatter）

| 技能/工具 | 用途 | 优先级 | 状态 |
|---|---|---|---|
| CTA 模板库 | 关注/在看/转发/评论各 5 种风格 | P0 | 内置 |
| 评论区运营 | 置顶话术+互动引导+负面应对 | P1 | 内置 |
| 私域转化路径 | 公众号→社群/个人号链路设计 | P2 | 内置 |
| 涨粉策略引擎 | 公众号/知乎/跨平台涨粉路径 | P1 | 内置 |
...
| **P1 强烈推荐** | 多 Agent 编排范式 | 秘书 | 📚 方法论参考 | Role/Task/Crew 抽象 |
| **P1 强烈推荐** | SOP 驱动结构化交接 | 秘书/大纲师 | 📚 方法论参考 | SOP 驱动 + 结构化交接 |
| **P1 强烈推荐** | 多角度提问检索写作 | 研究员/大纲师 | 📚 方法论参考 | 多视角提问 + 分节撰写 |
| **P1 强烈推荐** | Planning-Writing 双层分离 | 秘书/大纲师/撰稿人 | 📚 方法论参考 | Planning/Writing 双层分离 |
...
| **P2 可选参考** | 风格指纹量化 | 风格官 | 📚 方法论参考 | 中文文风量化，对应 QA 框架吸收 11 |
| **P2 可选参考** | Promptfoo | 审查员/秘书 | 📚 方法论参考 | CI/CD 防 prompt 回归，对应 QA 框架吸收 4 |
| **P0 必备** | 爆款标题公式库 | 写手 | 🔧 内置 | 8 大标题公式，Step 5 标题生成核心能力 |
| **P0 必备** | 四维归因分析 | 秘书 | 🔧 内置 | 选题/标题/内容/时间归因，Step 12 数据闭环核心 |
| **P0 必备** | 策略迭代引擎 | 秘书 | 🔧 内置 | 四方向优化建议，反馈闭环核心 |
| **P0 必备** | CTA 模板库 | 排版师 | 🔧 内置 | 关注/在看/转发/评论各 5 种风格 |
| **P0 必备** | 系列化内容规划 | 选题官 | 🔧 内置 | 系列主题设计+节奏控制 |

**状态图例**：✅ 已下载本地 ｜ 🌐 在线版可用 ｜ 🔌 API 接入 ｜ 📚 方法论参考 ｜ 🔧 内置 ｜ ⏳ 待集成 ｜ ⚠️ 不可访问
...
4. **方法论参考**（多 Agent 编排 / SOP 结构化 / 多角度提问检索 / Planning-Writing 双层分离 / 去 AI 味方法论 等）：只吸收设计思路
...
当前版本：v2.5（整合回 7 角色架构：十一节标题工程师技能归入 04-writer 写手标题技能、十二节数据分析师技能归入 00-secretary 秘书数据复盘技能、十三节运营策划师技能拆分归入 01-topic-scout 选题官运营技能 + 07-formatter 排版师运营技能；优先级速查表同步更新；对应 orchestration.md v1.4）
